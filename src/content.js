...
let current_page = "dfa-prompts-intro";
let index = 0;
let pages = ["motivation", "wanted", "sustainable", "return", "disposal", "dfa-congrats", "dfa-reflection"]
let default_responses = ["Building better habits starts with small steps.", 
                        "Always think through purchases before buying.", 
                        "You took the time today to be more sustainable!"];
let summaries = ["hmm, maybe reconsider this purchase or think about how it can be more sustainable!", 
                "this sounds like a good purchase, but we still recommend thinking about it longer!", 
                "this sounds like a sustainable, conscientious purchase! if you're sure, go for it!"];
...
function goForward() {
    ...
    if (current_page == "dfa-reflection"){
        document.getElementById("dfa-arrow-container").style.display = "none";
        let score = final_points.reduce((a, b) => a + b, 0);
        let summary = "";
        if (score < 4){
            summary = summaries[0];
        ...
        let possible_reponses = Array.from(final_responses);
